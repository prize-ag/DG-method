  program RKDG_WENO_SCALAR_LEGENDRE
    ...
    !double precision,parameter :: time_out = 2.0d0 /  pi !Final time of the solution
...
! 아직 안봄 
subroutine Get_Weights(Legen_ord,Q,i_bc,imax,x,dx,d)
  implicit none
  ...
  double precision,intent(out) :: d(0:Legen_ord,1:Legen_ord+2)
  ...
  !Kernels for the local polynomial
  double precision,dimension(0:Legen_ord,-Legen_ord:Legen_ord,1:Legen_ord+3)&
                                                               & :: L_K
  !Kernels for the global polynomial
  double precision,dimension(-Legen_ord:Legen_ord,1:Legen_ord+3) :: G_K
  !-------------------------Calculations have started--------------------------!
  
  i=1
  if((Legen_ord+1).eq.2)then
   !Third-order interpolation
    do quad=1,Legen_ord+1
   !Compute local kernels for each quadrature nodes on [-1,0]
      do l=-1,0
        L_K(0,l,quad)=Lag_C(l,-1,0,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [0,1]
      do l=0,1
        L_K(1,l,quad)=Lag_C(l,0,1,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute global kernels for each quadrature nodes on [-1,0,1]
      do l=-1,1
        G_K(l,quad)=Lag_C(l,-1,1,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute ideal weights
      d(0,quad)=G_K(-1,quad)/L_K(0,-1,quad)
      d(Legen_ord,quad)=G_K(1,quad)/L_K(1,1,quad)
    enddo
  elseif((Legen_ord+1).eq.3)then
   !Fifth-order interpolation
    do quad=1,Legen_ord+2
   !Compute local kernels for each quadrature nodes on [-2,-1,0]
      do l=-2,0
        L_K(0,l,quad)=Lag_C(l,-2,0,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [-1,0,1]
      do l=-1,1
        L_K(1,l,quad)=Lag_C(l,-1,1,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [0,1,2]
      do l=0,2
        L_K(2,l,quad)=Lag_C(l,0,2,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute global kernels for each quadrature nodes on [-2,-1,0,1,2]
      do l=-2,2
        G_K(l,quad)=Lag_C(l,-2,2,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute ideal weights
      d(0,quad)=G_K(-2,quad)/L_K(0,-2,quad)
      d(Legen_ord,quad)=G_K(2,quad)/L_K(2,2,quad)

      d(1,quad)=G_K(-1,quad)-d(0,quad)*L_K(0,-1,quad)
      d(1,quad)=d(1,quad)/L_K(1,-1,quad)
    enddo
  elseif((Legen_ord+1).eq.4)then
   !Seventh-order interpolation
    do quad=1,Legen_ord+1
   !Compute local kernels for each quadrature nodes on [-3,-2,-1,0]
      do l=-3,0
        L_K(0,l,quad)=Lag_C(l,-3,0,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [-2,-1,0,1]
      do l=-2,1
        L_K(1,l,quad)=Lag_C(l,-2,1,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [-1,0,1,2]
      do l=-1,2
        L_K(2,l,quad)=Lag_C(l,-1,2,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [0,1,2,3]
      do l=0,3
        L_K(3,l,quad)=Lag_C(l,0,3,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute global kernels for each quadrature nodes on [-3,-2,-1,0,1,2,3]
      do l=-3,3
        G_K(l,quad)=Lag_C(l,-3,3,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute ideal weights
      d(0,quad)=G_K(-3,quad)/L_K(0,-3,quad)
      d(Legen_ord,quad)=G_K(3,quad)/L_K(3,3,quad)

      d(1,quad)=G_K(-2,quad)-d(0,quad)*L_K(0,-2,quad)
      d(1,quad)=d(1,quad)/L_K(1,-2,quad)

      d(2,quad)=G_K(-1,quad)-d(0,quad)*L_K(0,-1,quad)&
                           &-d(1,quad)*L_K(1,-1,quad)
      d(2,quad)=d(2,quad)/L_K(2,-1,quad) 
    enddo
  elseif((Legen_ord+1).eq.5)then
   !Ninth-order interpolation
    do quad=1,Legen_ord+2
   !Compute local kernels for each quadrature nodes on [-4,-3,-2,-1,0]
      do l=-4,0
        L_K(0,l,quad)=Lag_C(l,-4,0,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [-3,-2,-1,0,1]
      do l=-3,1
        L_K(1,l,quad)=Lag_C(l,-3,1,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [-2,-1,0,1,2]
      do l=-2,2
        L_K(2,l,quad)=Lag_C(l,-2,2,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [-1,0,1,2,3]
      do l=-1,3
        L_K(3,l,quad)=Lag_C(l,-1,3,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute local kernels for each quadrature nodes on [0,1,2,3,4]
      do l=0,4
        L_K(4,l,quad)=Lag_C(l,0,4,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute global kernels for each quadrature nodes on [-4,-3,-2,-1,0,1,2,3,4]
      do l=-4,4
        G_K(l,quad)=Lag_C(l,-4,4,i_bc,imax,i,x,dx,Q(quad,1))
      enddo
   !Compute ideal weights
      d(0,quad)=G_K(-4,quad)/L_K(0,-4,quad)
      d(Legen_ord,quad)=G_K(4,quad)/L_K(4,4,quad)

      d(1,quad)=G_K(-3,quad)-d(0,quad)*L_K(0,-3,quad)
      d(1,quad)=d(1,quad)/L_K(1,-3,quad)
    
      d(2,quad)=G_K(-2,quad)-d(0,quad)*L_K(0,-2,quad)&
                           &-d(1,quad)*L_K(1,-2,quad)
      d(2,quad)=d(2,quad)/L_K(2,-2,quad)                   

      d(3,quad)=G_K(-1,quad)-d(0,quad)*L_K(0,-1,quad)&
                           &-d(1,quad)*L_K(1,-1,quad)&
                           &-d(2,quad)*L_K(2,-1,quad)
      d(3,quad)=d(3,quad)/L_K(3,-1,quad)
    enddo
  endif
...
subroutine Get_Residual(TVB_M,Legen_ord,Q,M,d,l_b,r_b,i_bc,imax,x,dx&
    ...
    double precision,dimension(1:imax) :: alp_u
